...
#     file on disk. This is needed because the engine's smart-cache hashes on
#     config/source/starting-cash, so tweaking any of those creates a NEW pkl
#     per symbol instead of overwriting the old one — without dedup, old runs
#     pile up and get double-counted in the portfolio aggregation below.

#     regime_name falls back to "unknown_regime" for pkl files written before
#     the engine started tagging results with their regime.
#     """
...
#             # Safely inject strategy name into the root of the dictionary
...
#             # Fallback for pkl files cached before regime tagging existed
...
#             # Keep only the most recently written file for this symbol+strategy+regime
...
#     `data_fingerprint` is a small, cheap-to-hash summary of _strat_data (ticker
#     count + total trade count). Without it, this cache was keyed only on
#     strategy_name — meaning re-running backtests and coming back to the same
#     strategy would silently serve stale numbers, since Streamlit had no way
#     to know the underlying data had changed.
...
#         # Take the final equity value for each trading day
...
#     # Fast Vectorized Concat & Resample
#     # Safe now: all symbols in _strat_data share the same regime's date range,
#     # so ffill/fillna no longer bridges across unrelated time windows.
...
# # Fast List Comprehension Filter
# strat_data_all_regimes = [r for r in raw_data if r.get("strategy_used", "Unknown") == selected_strategy]

# # 2. Regime Selector — NEW
# # Symbols from different regimes cover different, non-overlapping date windows.
# # Aggregating across regimes was producing misleading flat "shelf" plateaus in
# # the portfolio chart (ffill holding stale values, fillna padding pre-regime
# # gaps). Filtering to one regime at a time keeps the aggregation meaningful.
...
#     # Cheap fingerprint of the current data slice so the cache actually
#     # invalidates when trades/tickers change, instead of trusting only the name.
#     fingerprint = (
#         len(strat_data),
#         sum(len(r.get("trades", [])) for r in strat_data),
#     )

#     # Send to the hyper-optimized calculation engine
...
#         c1.metric("Net P&L", f"${t_data['net_pnl']:,.2f}", f"{t_data['net_pnl_pct']:+.2f}%")
#         c2.metric("Buy & Hold Benchmark", f"{t_data['buy_hold_pct']:+.2f}%")
#         c3.metric("Sharpe Ratio", f"{t_data['sharpe_ratio']:.2f}")
#         c4.metric("Max Drawdown", f"{t_data['max_dd_pct']:.2f}%", delta_color="inverse")
