...
# =====================================================================
# 4. MONTE CARLO SIMULATION (Now 100x Faster via Vectorization)
# =====================================================================
...
def run_monte_carlo_simulation(all_trades, starting_capital, avg_buy_hold_pct=0.0, spy_return_pct=None, num_simulations=1000, ruin_threshold_pct=20.0, title="ADVANCED MONTE CARLO STRESS TEST"):
    ...
    # 5. Fast Streak Calculation (Using a highly optimized 1D pass on boolean masks)
...
def run_backtest():
    ...
    print(f"🚀 Initializing Multi-Regime Backtest Engine...")
    print(f"⚙️ Cash/Ticker: ${control.STARTING_CASH_PER_TICKER:,.2f} | Provider: {control.HISTORICAL_PROVIDER}")
    print(f"🧠 Strategy: {control.ACTIVE_STRATEGY.__name__} | Res={bar_resolution} | Lookback={strategy_config['lookback']} | Sizing={strategy_config['position_mode']}")
    print(f"📋 Active Asset Class: {control.ACTIVE_ASSET_TYPE}\n")
    ...
    for regime_name, config in regimes.items():
        ...
        global_summary["total_ending_capital"] += total_ending_capital
        ...
        if total_trade_count > 0:
            mc_tasks.append({
                "title": f"MC STRESS TEST: {regime_name.upper()}",
                "all_trades": sorted_all_trades,
                "starting_capital": total_initial_capital,
                "avg_buy_hold_pct": avg_buy_hold_pct,
                "spy_return_pct": spy_metrics["bench_return_pct"] if spy_metrics else None
        ...
        print(f" Portfolio Sharpe Ratio:      {portfolio_sharpe:.2f}")
        print("=" * 115 + "\n")
        # NOTE: Individual Monte Carlo run has been explicitly removed from this loop.

    # =====================================================================
    # MASTER GLOBAL SUMMARY ACROSS ALL REGIMES
    # =====================================================================
    if global_summary["total_symbols_evaluated"] > 0:
        global_pnl = global_summary["total_ending_capital"] - global_summary["total_initial_capital"]
        global_pnl_pct = (global_pnl / global_summary["total_initial_capital"]) * 100 if global_summary["total_initial_capital"] > 0 else 0
        global_win_rate = (global_summary["total_wins"] / global_summary["total_trades"] * 100) if global_summary["total_trades"] > 0 else 0
        
        global_gp = global_summary["total_gross_profit"]
        global_gl = global_summary["total_gross_loss"]
        global_pf = (global_gp / global_gl) if global_gl > 0 else (float('inf') if global_gp > 0 else 0.0)
        
        avg_global_bh = np.mean(global_summary["bh_returns"]) if global_summary["bh_returns"] else 0.0

        print("\n" + "★" * 120)
        print(f"{'🌟 OVERALL MULTI-REGIME PERFORMANCE SUMMARY 🌟':^120}")
        print("★" * 120)

        # --- INDIVIDUAL REGIME RECAP TABLE (Now with Beta & Alpha) ---
        print(f"{'REGIME':<20} | {'NET P&L ($)':>11} | {'P&L (%)':>8} | {'B&H (%)':>8} | {'TRADES':>6} | {'WIN %':>6} | {'PF':>5} | {'SHARPE':>6} | {'MAX DD':>8} | {'BETA':>5} | {'ALPHA (%)':>9}")
        print("-" * 120)
        for rs in global_summary["regime_summaries"]:
            ...
            print(f"{r_name:<20} | ${rs['pnl']:>10,.2f} | {rs['pnl_pct']:>7.2f}% | {rs['bh_pct']:>7.2f}% | {rs['trades']:>6} | {rs['win_rate']:>5.1f}% | {rs['pf']:>4.2f} | {rs['sharpe']:>6.2f} | -{rs['max_dd_pct']:>6.2f}% | {beta_str} | {alpha_str}")
        print("-" * 120)
        ...
        print(f" Global Aggregate Net P&L:    ${global_pnl:+,.2f} ({global_pnl_pct:+.2f}%)")
        ...
        # =====================================================================
        if mc_tasks:
            print(f"🎲 Running Vectorized Monte Carlo Simulations for {len(mc_tasks)} Individual Regimes...")
        ...
        # NOTE: The "Advanced Monte Carlo for All Regimes" block has been explicitly removed as requested.
